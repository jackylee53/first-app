...
## **二、内置方法**
#### **1. re.match**
&emsp;&emsp;re.match 尝试从字符串的起始位置匹配一个模式，如果不是起始位置匹配成功的话，match()就返回none。
##### **语法：**
```text
...
参见：正则表达式修饰符 - 可选标志
###### 返回值：
&emsp;&emsp;匹配成功re.match方法返回一个匹配的对象，否则返回None。
...
None
```
- 第一个返回结果，我们可以看到匹配了www。
- 第二个返回结果，我们可以看到没有匹配项，返回None。
##### 获取匹配的值
