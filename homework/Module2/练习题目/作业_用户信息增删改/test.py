...
def show_employee_info():
    ...
    for i in range(len(title)):
        print(title[i].rjust(14),end=" ")
    print("")
    print("--".center(100, "-"))
    for item in user_list:
        for i in range(len(item)):
            print(item[i].rjust(15),end="")
    print("\n")
...
def query_operation(content):
    ...
    query_list_head, position1, position2, position3, position4         = query_list[0], query_list[-1], query_list[-2], query_list[-3], query_list[-4]  # 去除 * ,name/age
...
def update_operation(content):
    ...
    content_list = list_remove_null(content)  # 分解sql命令 [‘staff_table‘, ‘SET‘, ‘dept‘, ‘"Market"‘, ‘WHERE‘, ‘dept‘, ‘"IT"‘]
    if content_list[0] == "staff_table":  # 判读数据表,关键字例如where，set和要查询的字段例如name必须存存在(合法)
        # 判断sql 语法 合法性
        if content_list[1].lower() in update_keywords and content_list[4].lower() in update_keywords and                         content_list[2].lower() in field_keywords and content_list[5] in field_keywords:
