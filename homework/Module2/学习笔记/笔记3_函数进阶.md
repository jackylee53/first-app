...
```text
select name,age from employee_info where age > 22
select * from employee_info where dept = "IT"
select * from employee_info where enroll_date like "2013"
```
...
```text
del from employee_info where id = 3
```
4. 可修改员工信息，语法如下:
```text
update employee_info set dept = Market where dept = IT #把所有dept=IT的纪录的dept改成Market
update employee_info set age = 25 where name = Alex Li #把name=Alex Li的纪录的年龄改成25
```
